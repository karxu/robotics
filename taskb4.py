...
def turn(side):
    ...
    if (side%2 == 0):
       ...
       #turn 90 degrees
       motorl.run_timed(duty_cycle_sp = 80, time_sp = 500)   #experimented with hard value here, 10:-20. 30:-20, 30:0, (too small) 100|:0, 100:-50, (overshooting) 90:-30(best), 45:0, 50:0, 70:0, 80,0, 70:-10
       motorr.run_timed(duty_cycle_sp = 0, time_sp = 500)  #and the time stamp 100 1000 10000 500(best time)
       time.sleep(1)
       detectnewline()
       motorl.run_timed(duty_cycle_sp = 0, time_sp = 500)
       motorr.run_timed(duty_cycle_sp = 80, time_sp = 500)
       time.sleep(1)
    else:
       ...
       time.sleep(1)
       detectnewline()
       motorl.run_timed(duty_cycle_sp = 80, time_sp = 500)
       motorr.run_timed(duty_cycle_sp = 0, time_sp = 500)
       time.sleep(1)
    return
...
def detectnewline():
    ...
    while (c.value() > 20):
        ...
        time.sleep(.1)
    motorl.run_timed(duty_cycle_sp = -5, time_sp=20)
    motorr.run_timed(duty_cycle_sp = -5, time_sp=20)
    time.sleep(1)
    return
